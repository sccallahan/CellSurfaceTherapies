...
counter <- 1
start <- Sys.time()
tmp$response_zero <- sapply(tmp[[3]], function(desc) get_response2_count_temp0(text = desc, model = "phi4"))
stop <- Sys.time()
time <- stop - start
time

saveRDS(object = tmp, file = here('data/processed/nci_dd_target_guess_LLM_phi4_tempzero.rds'))

## refine answers ----
to_refine <- readRDS(here('data/processed/nci_dd_target_guess_LLM_phi4_tempzero.rds'))
to_refine <- to_refine %>% 
  mutate(response = gsub("\\s+", "|", response_zero)) %>%
...
counter <- 1
start <- Sys.time()
to_refine$response_refine <- sapply(to_refine[[4]], function(desc) refine_response2_temp0(text = desc, model = "phi4"))
stop <- Sys.time()
time <- stop - start
time
Sys.time()
...
saveRDS(object = to_refine_final, file = here('data/processed/nci_dd_target_guess_LLM_phi4_refined_tempzero.rds'))

#~~~~~~~~~~~~~~~~~~#
# phi4 clin trials interventions ----
#~~~~~~~~~~~~~~~~~~#
## clean studies ----
...
counter <- 1
start <- Sys.time()
tmp$response_drugname <- sapply(tmp[[1]], function(desc) get_response2_count_drugname_temp0(text = desc, model = "phi4"))
stop <- Sys.time()
time <- stop - start
time
Sys.time()

saveRDS(object = tmp, file = here('data/processed/clinical_trial_target_guess_LLM_phi4_drugname_tempzero.rds'))

## refine ----
counter <- 1
start <- Sys.time()
tmp$response_refine <- sapply(tmp[[2]], function(desc) refine_response2_temp0(text = desc, model = "phi4"))
stop <- Sys.time()
time <- stop - start
time
Sys.time()
...
saveRDS(object = tmp, file = here('data/processed/clinical_trial_target_guess_LLM_phi4_drugname_refined_tempzero.rds'))