...
write_get_therapy_drugdef_temp0_jsonl <- function(df, file, column) {
  con <- file(file, open = "w")
  on.exit(close(con))
    ...
    messages <- list(
      ...
      list(
        ...
        content = paste0(
          ...
          In cases where there is more than one therapy, return the answer separated by the pipe character (|).
          If the therapy is not one of the above, return NA. Do not include any explanation or commentary.
          The drug description/definition is as follows: ", definition
        )
      )
    )
...
chunk_size <- 40
num_chunks <- ceiling(nrow(studies_guess) / chunk_size)

df_chunks <- split(studies_guess, rep(1:num_chunks, each = chunk_size, length.out = nrow(studies_guess)))

path <- here('data/openai_chunks_therapies_curated/')
...
num_chunks_nci <- ceiling(nrow(final_dictionary_definitions) / chunk_size)

df_chunks_nci <- split(final_dictionary_definitions, rep(1:num_chunks_nci, each = chunk_size, length.out = nrow(final_dictionary_definitions)))

path_nci <- here('data/openai_chunks_therapies_nci/')
for (i in seq_along(df_chunks_nci)) {
  chunk <- df_chunks_nci[[i]]
  file_name <- sprintf("nci_trial_therapy_guess_definition_chunk_%d.jsonl", i)
  file_name_full <- paste0(path_nci, file_name)
  write_get_therapy_drugdef_temp0_jsonl(chunk, file_name_full, column = 3)
}

for (i in seq_along(df_chunks_nci)) {
  chunk <- df_chunks_nci[[i]]
  file_name <- sprintf("nci_trial_therapy_guess_drug_chunk_%d.jsonl", i)
  file_name_full <- paste0(path_nci, file_name)
  write_get_therapy_drug_temp0_jsonl(chunk, file_name_full, column = 2)
}
...
# RUN 02_CALL_THERAPY_TYPES_GPT40_SUBMISSION.PY HERE